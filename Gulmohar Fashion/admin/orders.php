<?php
  ...
  if(isset($_GET['complete']) && $_GET['complete'] == 1){
    ...
    $db->query("UPDATE cart SET shipped = 1 WHERE cart_id = '{$cart_id}'");
    $_SESSION['success_flash'] = 'The Order has been Completed';
    header('Location: index.php');
  }

  $txn_id = sanitize((int)$_GET['txn_id']);
  $txnQuery = $db->query("SELECT * FROM transactions WHERE transactions_id = '{$txn_id}'");
  ...
  $ids = implode(',',$idArray);
  $productQ = $db->query("
    SELECT i.products_id as 'id', i.title as 'title', c.categories_id as 'cid', c.category as 'child', p.category as 'parent'
    FROM products i LEFT JOIN categories c ON i.categories = c.categories_id
    LEFT JOIN categories p ON c.parent = p.categories_id where i.products_id IN ({$ids})
  ");
   ...
   <div class="col-md-6">
     ...
     <table class="table table-condensed table-striped table-bordered">
       <tbody>
         ...
         <tr>
           <td>Order Date</td>
           <td><?=formatted_date($txn['txn_date']);?></td>
         </tr>
       </tbody>
     </table>
   </div>
