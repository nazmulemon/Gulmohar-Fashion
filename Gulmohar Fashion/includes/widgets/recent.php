...
<?php
  $transQ = $db->query("SELECT * FROM cart WHERE paid = 1 ORDER BY cart_id DESC LIMIT 5");
  $results = array();
...
<div id="recent_widget">
  <table class="table table-condensed">
      <?php foreach($used_ids as $id):
        $productQ = $db->query("SELECT products_id,title FROM products WHERE products_id = '{$id}'");
